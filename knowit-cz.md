---
layout: cv
title: Jiri Weiser's resume
---

# Jiří Weiser

Technical Mentor & Educator

* jiri.weiser-at-knowit.cz
* [LinkedIn](https://www.linkedin.com/in/jiri-weiser/)

## About Me

Senior Engineer with 10+ years in high-performance parallel C++ systems. I love tightening the gap between complex technical concepts and team capabilities through mentorship, workshops, and AI integration. I believe the best performing teams are built on open communication, continuous learning, and asking the right questions.

## Competencies

* **Architecture & Design:** High-performance systems, scalability, multi-language systems (C++, Python, Java), legacy system modernization
* **Team Leadership:** Technical coaching, mentoring, knowledge sharing
* **AI Integration:** LLM integrations, AI-assisted development, process automation
* **Technical Enablement:** Designing and delivering custom workshops tailored to specific team skill gaps.

## Experience Highlights

### KnowIT CZ

`since 2021`

__Co-founder, Technical Mentor & Educator__

* **Process Optimization:** Established sustainable development workflows for client teams, helping them shift from reactive "crunch modes" to consistent, predictable delivery cycles.
* **Team Growth Strategies:** Designed personalized technical growth plans for engineers, aligning their individual strengths and wishes with company goals.
* **Baseline Standardization:** Helping companies establish a solid baseline of technical practices (Clean Code, Architecture) to ensure long-term maintainability.
* **AI Workshops:** Conducting practical workshops on integrating AI into development processes, focusing on real-world coding efficiency rather than just theory.

### Thermo Fisher Scientific

`2 years`

__Technical Lead__

* **Bridging Domains:** Built and led a software engineering team from the ground up, mentoring domain experts (physicists, mathematicians, biologists) to adopt professional software engineering standards.
* **Complex Architecture:** Architected a physical simulation platform deployed simultaneously in three distinct forms: a standalone Desktop GUI, a Microservice, and a Python-integrated library.
* **Hands-on Engineering:** Acted as the primary hands-on developer during the team's formation, setting up CI/CD infrastructure and release pipelines.
* **AI Integration:** Pioneered the use of AI tools to bring efficiency to development processes.

### Masaryk University

`since 2011`

__Lecturer__

* Teaching programming courses in C, C++, Python, and Java.
* Created and taught a course on system design and software architecture, putting academic theory into practical use for 100+ students

## Employment History

* **Mavenir** - Senior Software Engineer
* **Thermo Fisher Scientific** - Technical Lead
* **NetSuite / Oracle** - Software Engineer
* **Seznam.cz** - Software Engineer

## Academic Background

* **Researcher:** Parallelism & Verification at Masaryk University, Faculty of Informatics
* **Master's Degree:** Computer Science, more details at <https://is.muni.cz/person/374154>
